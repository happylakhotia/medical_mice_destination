...
export default function MailBox({params}: {params: {rootid: string}}) {
...
function MailList({ mails, type, highlightedMails, rootid }: {
  mails: Mail[];
  type: "Received" | "Sent";
    ...
    <Card>
      ...
      <CardContent>
        <ScrollArea className="h-[600px]">
          {mails.map((mail) => (
            <Link href={`/dashboard/doctor/${rootid}/mailbox/${mail.id}`} key={mail.id}>
              <div
                key={mail.id}
